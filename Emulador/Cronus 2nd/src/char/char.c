...
static int chardb_waiting_disconnect(int tid, unsigned int tick, int id, intptr_t_t data);
...
// Char structure of a version 1 (original data structure)
	...
	for(i = 0; str[next] && str[next] != '\t' && str[next] != '\n' && str[next] != '\r'; i++) { // global_reg?????athena.txt???????'\n'????
...
/// If all the conditions are met, it stops the core loop.void loginif_check_shutdown(void)
